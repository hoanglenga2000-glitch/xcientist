...
Env vars (all optional at import time, validated on use):
  ...
  GPU_SSH_KEY_PATH[_FILE] / GPU_SSH_KNOWN_HOSTS_PATH[_FILE]
...
def connect_ssh(config: Optional[GpuSshConfig] = None, *, timeout: int = 20):
    ...
    if config.known_hosts_path:
        known_hosts = Path(config.known_hosts_path).expanduser()
        if not known_hosts.is_file():
            raise CredentialError("GPU_SSH_KNOWN_HOSTS_PATH points to a missing file (path hidden)")
