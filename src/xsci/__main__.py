...
from .config import GLOBAL_DIR, find_project_dir, load_config
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ini.add_argument(
        "--compute",
        choices=_PUBLIC_COMPUTE_CHOICES,
        default="gpu",
        help="compute backend for this project (HPC/GPU only)",
    )
    ...
    log.add_argument("--provider", choices=["anthropic", "deepseek", "openai"], help="LLM provider")
    ...
    log.add_argument("--non-interactive", action="store_true",
                     help="read secret values from provider/Kaggle environment variables")
    ...
    run.add_argument(
        ...
        choices=_PUBLIC_COMPUTE_CHOICES,
        default="gpu",
        help="compute backend (HPC/GPU only)",
    )
    run.add_argument("--iterations", type=int, help="number of search iterations")
    run.add_argument("--data-dir", default="", help="remote HPC data directory or task-relative name")
    ...
    agt.add_argument("--data-dir", default="", help="remote HPC data directory or task-relative name")
...
def cmd_config(args: argparse.Namespace) -> int:
    cfg = load_config()
    if args.key:
        if args.key.casefold() == "secrets" or args.key.casefold().startswith("secrets."):
            print("(redacted: secret configuration is never displayed)")
            return 0
        val = cfg.get(args.key)
        print(val if val is not None else f"(unset: {args.key})")
        return 0 if val is not None else 1
    print("config sources (low->high precedence):")
    ...
    print(f"compute.backend : {cfg.get('compute.backend', 'gpu (default)')}")
    return 0
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if contract_blocks_training(contract, require_model_ready=True):
        ...
        return 1
    print("\nstarting research loop (this spends API tokens and uses the remote HPC/GPU) ...\n")
...
def cmd_report(args: argparse.Namespace) -> int:
    from .results import find_result, list_results
    if not args.run:
        rows = list_results()
        if not rows:
