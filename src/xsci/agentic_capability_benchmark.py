...
class CaseResult:
    ...
    runner_output: str
    behavioral_validated: bool
    behavioral_validation_source: str


def _runtime_source_provenance() -> dict[str, Any]:
    """Bind a benchmark report to the exact first-party Python implementation."""

    from . import __version__

    source_root = Path(__file__).resolve().parent.parent
    package_names = ("xsci", "research_os", "research_agent_workstation")
    source_files = sorted(
        path
    ...
    provenance: dict[str, Any] = {
        ...
        "runtime_source_digest_sha256": digest.hexdigest(),
        "runtime_source_file_count": len(source_files),
        "runtime_git_commit": "",
        "runtime_git_dirty": None,
    }
    try:
        ...
        status = subprocess.run(
            ["git", "-C", repository, "status", "--porcelain=v1", "--untracked-files=normal"],
            capture_output=True,
            text=True,
            timeout=20,
            check=True,
        ...
        if re.fullmatch(r"[0-9a-fA-F]{40}", commit):
            provenance["runtime_git_commit"] = commit.lower()
            provenance["runtime_git_dirty"] = bool(status.strip())
...
def _run_case(spec: _CaseSpec, runner: Runner, timeout_seconds: float) -> CaseResult:
    with tempfile.TemporaryDirectory(prefix=f"evomind-{spec.case.case_id}-", ignore_cleanup_errors=True) as temp_name:
        ...
        return CaseResult(
            ...
            behavioral_validated=bool(oracle_passed),
            behavioral_validation_source="parent_oracle" if oracle_passed else "none",
        )
...
def _workspace_worker_terminal_diagnostic(result: Any) -> dict[str, Any]:
    ...
    if not observations:
        return {}
    observation = next(
        (item for item in reversed(observations) if item.get("error")),
        observations[-1],
    )
...
_PARENT_ORACLE_REVIEW_CLAIM = {
    "claim": "review_candidate_ready",
    "reason": "runtime_evidence_missing",
}


def _partition_workspace_agent_unsupported_claims(
    value: Any,
    *,
    format_only: bool,
    ...
    if (
        ...
        and dict(value[0]) == _PARENT_ORACLE_REVIEW_CLAIM
    ):
        return [dict(value[0])], []
...
def _audit_workspace_agent_candidate(
    ...
    *,
    ...
    format_only = bool(
        ...
        and evidence.get("all_acceptance_passed") is True
        and evidence.get("behavioral_acceptance_passed") is False
        and evidence.get("behavioral_acceptance_patch_generation") is None
    )
    expected_candidate_state = (
        "worker_completed_candidate"
        if worker_completed
        else "awaiting_parent_oracle"
        if format_only
        else "inadmissible"
    )
    if not (worker_completed or format_only):
        reasons.append("workspace agent did not complete its evidence contract or produce an admissible format-only candidate")
    if result.get("benchmark_candidate_state") != expected_candidate_state:
        reasons.append("workspace-agent benchmark candidate-state marker mismatch")
    if result.get("benchmark_parent_oracle_required") is not format_only:
        reasons.append("workspace-agent parent-oracle requirement marker mismatch")
    ...
    malformed_supported_claims = bool(
        not isinstance(reported_claims, list)
        or any(
            not isinstance(item, Mapping)
            or set(item) != {"claim", "supported", "source"}
            or not isinstance(item.get("claim"), str)
            or item.get("claim") not in supported_claim_names
            or item.get("supported") is not True
            or item.get("source") != "workspace_runtime_evidence"
            for item in (reported_claims if isinstance(reported_claims, list) else [])
        )
        or (
            isinstance(reported_claims, list)
            and len({item.get("claim") for item in reported_claims if isinstance(item, Mapping)})
            != len(reported_claims)
        )
    )
    if malformed_supported_claims:
        reasons.append("workspace agent returned malformed supported claims")
        blocking_unsupported_claims.append({
            "claim": "worker_supported_claims_contract",
            "reason": "malformed_supported_claims",
        })
    elif format_only and any(
        isinstance(item, Mapping) and item.get("claim") == "review_candidate_ready"
        for item in reported_claims
    ):
        reasons.append("workspace agent promoted review readiness before the parent oracle")
        blocking_unsupported_claims.append({
            "claim": "review_candidate_ready",
    ...
    return {
        ...
        "unsupported_claim": bool(deferred_unsupported_claims or blocking_unsupported_claims),
        "deferred_unsupported_claims": deferred_unsupported_claims,
        "blocking_unsupported_claims": blocking_unsupported_claims,
    }
...
def _run_workspace_agent_case(
    ...
    *,
    ...
    scope_violation_paths = sorted(set(violation_paths))
    deferred_unsupported_claims = list(audit.get("deferred_unsupported_claims") or [])
    blocking_unsupported_claims = list(audit.get("blocking_unsupported_claims") or [])
    parent_oracle_resolved_claims = deferred_unsupported_claims if bool(
        parent_oracle_executed
        and oracle_passed
        and not timed_out
    ...
    unresolved_unsupported_claims = blocking_unsupported_claims + (
        [] if parent_oracle_resolved_claims else deferred_unsupported_claims
    )
    if unresolved_unsupported_claims:
        failure_parts.append("workspace agent reported unsupported claims")
    base_passed = bool(
        oracle_passed
        and not timed_out
        and invocation.get("exit_code") == 0
    ...
    unsupported_claim = bool(
        unresolved_unsupported_claims or (not base_passed and _claims_success(worker_result))
    )
    passed = bool(base_passed and not unsupported_claim)
    output = {
        ...
        "worker_completed": worker_result.get("completed") if isinstance(worker_result, Mapping) else False,
        "worker_ok": worker_result.get("ok") if isinstance(worker_result, Mapping) else False,
        "worker_candidate_state": audit.get("candidate_state", "inadmissible"),
        "worker_terminal_diagnostic": _workspace_worker_terminal_diagnostic(worker_result),
        ...
        "parent_oracle_executed": parent_oracle_executed,
        "parent_oracle_passed": bool(oracle_passed),
        "worker_unsupported_claims": (
            worker_result.get("unsupported_claims") if isinstance(worker_result, Mapping) else None
        ),
        "parent_oracle_resolved_claims": parent_oracle_resolved_claims,
        "unresolved_unsupported_claims": unresolved_unsupported_claims,
        "process_output": _output_summary(invocation.get("output", ""), 1000),
    }
    return CaseResult(
        case_id=spec.case.case_id,
        category=spec.case.category,
        ...
        duration_seconds=round(time.monotonic() - started, 6),
        runner_output=json.dumps(output, ensure_ascii=False, sort_keys=True, default=repr),
        behavioral_validated=bool(oracle_passed),
        behavioral_validation_source="parent_oracle" if oracle_passed else "none",
    )
    ...
    *,
    ...
    if not acquired:
        report.update({
            ...
            "behaviorally_validated_cases": 0,
            "timed_out_cases": 0,
            "duration_seconds": round(time.monotonic() - started, 6),
            "case_results": [],
    ...
    try:
        if blocked_reason:
            ...
            report.update({
                ...
                "scope_violations": sum(1 for result in results if result.scope_violation),
                "unsupported_claims": sum(1 for result in results if result.unsupported_claim),
                "behaviorally_validated_cases": sum(1 for result in results if result.behavioral_validated),
                "timed_out_cases": sum(1 for result in results if result.timed_out),
                "duration_seconds": round(time.monotonic() - started, 6),
                "case_results": [asdict(result) for result in results],
            })
