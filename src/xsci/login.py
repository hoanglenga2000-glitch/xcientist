...
Windows secrets use current-user DPAPI; other platforms use ~/.xsci/secrets.toml
with 0600 permissions. Non-secret choices go to ~/.xsci/config.toml. Prompting
is separated from persistence so save logic remains unit-testable. Automated
setup reads secrets from environment variables rather than command-line values.
"""
...
def save_llm_credentials(provider: str, api_key: str, *, base_url: Optional[str] = None,
    ...
    ``provider`` records the actual provider (Anthropic, DeepSeek, or OpenAI).
    ``model`` is the user's chosen
...
def _prompt_llm() -> bool:
    ...
    print(f"  saved LLM key for {provider} to protected user storage")
    return True
...
def _prompt_kaggle() -> bool:
    ...
    print("  saved Kaggle credentials to protected user storage")
    return True
...
def run_login(
    *,
    ...
    """Interactive by default; automation reads secret values from env vars."""
    did_something = False

    if non_interactive:
        if api_key or kaggle_key:
            print("command-line secret values are disabled; use provider/Kaggle environment variables")
            return 1
        provider_key = os.environ.get(f"{str(provider or '').upper()}_API_KEY") if provider else ""
        env_kaggle_username = os.environ.get("KAGGLE_USERNAME", "")
        env_kaggle_key = os.environ.get("KAGGLE_KEY", "")
        if provider and provider_key:
            save_llm_credentials(provider, provider_key, base_url=base_url)
        ...
        if kaggle_json:
            ...
            did_something = True
        elif (kaggle_username or env_kaggle_username) and env_kaggle_key:
            save_kaggle_credentials(kaggle_username or env_kaggle_username, env_kaggle_key)
            print("saved Kaggle credentials")
            did_something = True
        if not did_something:
            print("nothing to do: set the selected provider API key and/or Kaggle environment variables")
            return 1
        return 0

    print("xsci login - keys use protected user storage and never enter your project.\n")
    try:
