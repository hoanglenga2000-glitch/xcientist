...
class TerminalAgent:
    ...
    def _handle_execution(self, raw: str, intent, session: SessionState,
        ...
        emitter = self._get_emitter(root)
        effective_compute = str(compute or session.compute_backend or "gpu").strip().lower()
        try:
        ...
        if session.gpu_blocked:
            ...
            emitter.emit("Selecting compute", "compute=gpu", status="passed")
