...
def _data_status(task: dict[str, Any]) -> dict[str, Any]:
    ...
    remote_dir = str(task.get("gpu_data_dir") or task.get("remote_data_dirname") or "")
    result: dict[str, Any] = {
...
def _build_proposals(task: dict[str, Any], data: dict[str, Any], recent: list[dict[str, Any]],
    ...
    if not data.get("train_csv") and not data.get("remote_data_dir"):
        return [
...
def build_scientist_checkpoint(session: SessionState, root: Path) -> dict[str, Any]:
    ...
    remote_data_ready = bool(data.get("remote_data_dir")) and effective_compute == "gpu"
    ...
    hypotheses = _build_research_hypotheses(task, data, recent, memory_records, blockers)
    ...
    return {
        ...
        "observe": observe,
        "analyze": analyze,
        "propose": _build_proposals(task, data, recent, blockers),
...
def build_research_decision(session: SessionState, root: Path, *,
    ...
    elif checkpoint.get("gate", {}).get("blockers"):
        blockers = checkpoint.get("gate", {}).get("blockers") or []
        decision = {
    ...
    elif not checkpoint.get("gate", {}).get("can_execute"):
