...
Design:
  * The *runner* is pluggable. Release builds require a gated HPC/GPU runner;
    ``LocalSubprocessRunner`` remains only as a fail-closed compatibility shim.
...
class LocalSubprocessRunner:
    """Deprecated compatibility shim; release builds never execute locally."""
    ...
    def run(self, code: str, *, data_dir: str, out_dir: str, exp_id: str) -> RunResult:
        from .hpc_policy import HPCPolicyError

        raise HPCPolicyError(
            "blocked_local_training_disabled: LocalSubprocessRunner is unavailable in release builds; "
            "configure the gated HPC/GPU runner"
        )
