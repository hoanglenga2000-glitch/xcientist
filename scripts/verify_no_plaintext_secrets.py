...
LAUNCH_CRITICAL_ROOTS = [
    ...
    ROOT / "scripts" / "verify_workstation_launch_readiness.py",
    ROOT / "scripts" / "verify_new_user_release_readiness.py",
    ROOT / "scripts" / "verify_no_plaintext_secrets.py",
    ROOT / "scripts" / "workstation_local_auth.py",
    ROOT / "src",
    ROOT / "prompts",
    ROOT / "benchmark",
...
TEST_FIXTURE_VALUE_PATTERN = re.compile(r"(?i)^(?:.*[-_])?test(?:[-_].*)?$")
...
CREDENTIAL_METADATA_NAME_PATTERN = re.compile(
    r"(?i)(?:^secret_scan$|^secrets$|status|policy|storage|username|user_name|path|file|dir|directory|env|name)$"
)
...
def _is_placeholder_value(text: str) -> bool:
    ...
    return (
        ...
        or bool(TEST_FIXTURE_VALUE_PATTERN.fullmatch(value))
...
def discover_launch_critical_files(root: Path = ROOT) -> list[Path]:
    candidates: set[Path] = set()
    for launch_root in LAUNCH_CRITICAL_ROOTS:
        if not launch_root.exists():
            continue
        if launch_root.is_file():
            candidates.add(launch_root)
            continue
        for current_root, dirnames, filenames in os.walk(launch_root):
            dirnames[:] = [
                dirname for dirname in dirnames if dirname.casefold() not in LAUNCH_SKIP_DIRS
            ]
            for filename in filenames:
                if ".test." in filename.casefold() or filename.casefold().startswith("test_"):
                    continue
                candidates.add(Path(current_root) / filename)
    return sorted(candidates, key=lambda item: _relative_name(item, root).casefold())


def scan_launch_critical_inventory(root: Path = ROOT) -> tuple[list[Finding], int]:
    findings: list[Finding] = []
    scanned_files = 0
    for path in discover_launch_critical_files(root):
        file_findings, scanned = scan_file(path, root)
        findings.extend(file_findings)
        scanned_files += int(scanned)
    return findings, scanned_files
...
def main(root: Path = ROOT, *, history: bool = False) -> None:
    root_is_project_default = root.resolve(strict=False) == ROOT.resolve(strict=False)
    if root_is_project_default and not history:
        findings, scanned_files = scan_launch_critical_inventory(root)
        mode = "launch_critical"
    else:
        findings, scanned_files = scan_repository(root)
        mode = "strict_repository"
