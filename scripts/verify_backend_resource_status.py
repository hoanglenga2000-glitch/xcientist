...
def configured_state_is_acceptable(key: str, state: str) -> bool:
    ...
    if any(marker in normalized for marker in ("blocked", "failed", "error")):
        return False
    if key == "kaggle":
        # A protected credential may be configured without having passed a real
        # API call. Both states are truthful and actionable; only authenticated
        # is runtime-ready.
        return normalized in {"configured_unverified", "authenticated"}
    if any(marker in normalized for marker in ("unverified", "not verified", "not ready", "pending")):
        return False
...
def main() -> None:
    ...
    if env_keys.get("KAGGLE_TOOLCHAIN_STATUS") != "ready" or env_keys.get("KAGGLE_TOKEN_STATUS") not in {
        "not_configured",
