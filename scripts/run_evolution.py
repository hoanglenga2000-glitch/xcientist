#!/usr/bin/env python3
"""Run the evolution engine on one task through the gated HPC/GPU runtime.

One command unifies both tracks:

    # GPU (real remote training; runtime contract must be configured)
...
from xsci.config import inject_engine_env, load_config  # noqa: E402

# Resolve secrets from the protected user store (DPAPI on Windows). Existing
# process environment variables remain authoritative for ephemeral CI/runtime use.
inject_engine_env(load_config(project_root=ROOT), override=False)

from research_os.evolution_loop import EvolutionConfig, EvolutionLoop  # noqa: E402
from research_os.hpc_policy import require_hpc_compute  # noqa: E402
...
from research_os.variation_generator import TaskContext  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--runner", choices=["gpu"], default="gpu")
    ap.add_argument("--iterations", type=int, default=5)
    ap.add_argument("--data-dir", default="", help="remote data path or dirname under the configured HPC workspace")
    ...
    require_hpc_compute(args.runner)
