...
FRESH_INSTALL_OPTIONAL_COMMAND_IDS = {
    "workstation_training_progress",
    "learning_loop_readiness",
}


def with_runtime_base_url(item: dict[str, Any], base_url: str) -> dict[str, Any]:
    resolved = {**item, "cmd": [*item["cmd"]]}
    flag = RUNTIME_BASE_URL_FLAGS.get(item["id"])
    if not flag:
        return resolved
    resolved["cmd"].extend([flag, base_url])
    if item["id"] == "server_health":
...
def with_fresh_install_policy(item: dict[str, Any], fresh_install: bool) -> dict[str, Any]:
    resolved = {**item, "cmd": [*item["cmd"]]}
    if fresh_install and item["id"] in FRESH_INSTALL_OPTIONAL_COMMAND_IDS:
        resolved["critical"] = False
        resolved["claim"] = f"{item['claim']} A fresh install may report no historical training evidence."
    return resolved


API_PATHS = [
    ("/api/workstation-summary", {200}),
    ("/api/tasks", {200}),
    ("/api/settings", {200}),
    ("/api/paper-evidence-bundle", {200, 404}),
]
...
def http_check(base_url: str, path: str, accepted_statuses: set[int] | None = None) -> dict[str, Any]:
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for item in COMMANDS:
        resolved = with_runtime_base_url(item, args.base_url)
        resolved = with_fresh_install_policy(resolved, args.fresh_install)
        checks.append(run_command(resolved, timeout=args.command_timeout))

    page_smoke = [http_check(args.base_url, f"/?page={page}") for page in PAGES]
    api_smoke = [http_check(args.base_url, path, accepted) for path, accepted in API_PATHS]
    ...
    if args.fresh_install and any(
        item["id"] in FRESH_INSTALL_OPTIONAL_COMMAND_IDS and item.get("status") != "passed"
        for item in checks
    ):
        blockers.append("fresh_install_training_evidence_absent")
