...
def gate_security_invariants(quiet: bool) -> tuple[bool, str]:
    verifier = ROOT / "scripts" / "verify_security_invariants.py"
    proc = subprocess.run(
    ...
    if proc.returncode == 0:
        return True, "security invariants passed"
    output = ((proc.stdout or "") + (proc.stderr or "")).strip()
    return False, output[:1200] or f"security verifier exited with code {proc.returncode}"


def gate_ui_truthfulness(quiet: bool) -> tuple[bool, str]:
    verifier = ROOT / "scripts" / "verify_workstation_ui_truthfulness.py"
    proc = subprocess.run(
        [sys.executable, str(verifier)],
        cwd=str(ROOT),
        capture_output=True,
    ...
    if proc.returncode == 0:
        return True, "runtime UI truthfulness gate passed"
    output = ((proc.stdout or "") + (proc.stderr or "")).strip()
    return False, output[:1600] or f"UI truthfulness verifier exited with code {proc.returncode}"


def gate_benchmark_registry(quiet: bool) -> tuple[bool, str]:
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    if str(SRC) not in sys.path:
        sys.path.insert(0, str(SRC))
    try:
        from research_os.benchmark_manager import load_tasks
        from scripts.mle_bench_split75_contract import verify_split75_contract

        registry_path = ROOT / "benchmark" / "mle_bench_75" / "tasks_template.json"
        upstream_path = ROOT / "benchmark" / "mle_bench_75" / "UPSTREAM.json"
        manifest_path = ROOT / "benchmark" / "mle_bench_75" / "openai_split75_507f92e.txt"
        tasks = load_tasks(registry_path)
        contract = verify_split75_contract(
            manifest_bytes=manifest_path.read_bytes(),
            upstream=json.loads(upstream_path.read_text(encoding="utf-8")),
            registry=json.loads(registry_path.read_text(encoding="utf-8")),
            runner_source=(ROOT / "scripts" / "mlebench_server_runner.py").read_text(encoding="utf-8"),
            rules_source=(ROOT / "scripts" / "accept_kaggle_rules_all75.py").read_text(encoding="utf-8"),
        )
    except Exception as exc:  # noqa: BLE001
        return False, f"benchmark registry rejected: {type(exc).__name__}: {exc}"
    return True, (
        f"{len(tasks)} local tasks; pinned official split={contract['official_tasks']}; "
        f"local overlap={len(contract['local_official_overlap'])}"
    )
...
def main() -> int:
    ...
    gates = [
        ...
        ("security", gate_security_invariants),
        ("ui-truth", gate_ui_truthfulness),
        ("registry", gate_benchmark_registry),
