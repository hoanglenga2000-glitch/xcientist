...
def main() -> None:
    ...
    if not args.python_executable:
        args.python_executable = (
            f"{args.remote_root.rstrip('/')}/pyenvs/tabular_s6e6_lightgbm/bin/python"
        )
    ...
    default_artifact_dir = ROOT / "workspace" / "hpc_experiments" / "playground_series_s6e6" / f"EXP016_lgbm_optuna_{mode}_{run_id}"
    local_artifact_dir = Path(args.local_artifact_dir).expanduser() if args.local_artifact_dir else default_artifact_dir
    if not local_artifact_dir.is_absolute():
        local_artifact_dir = ROOT / local_artifact_dir
    local_artifact_dir = local_artifact_dir.resolve()
    if not local_artifact_dir.is_relative_to((ROOT / "workspace").resolve()):
        parser.error("--local-artifact-dir must stay under repository workspace/")
    ...
    try:
        ...
        if remote_existing_data and not all(
            remote_file_exists(sftp, f"{remote_existing_data}/{name}") for name in data_names
        ):
            raise RuntimeError("--remote-data-dir is missing required Kaggle data files")
