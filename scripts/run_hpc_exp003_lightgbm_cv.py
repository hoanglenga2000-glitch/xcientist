...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run EXP003 LightGBM CV on the verified HPC SSH path.")
    ...
    remote_dir = f"{args.remote_root.rstrip('/')}/playground_series_s6e6/EXP003_{mode}_{run_id}"
    local_artifact_dir = ROOT / "workspace" / "hpc_experiments" / "playground_series_s6e6" / f"EXP003_{mode}_{run_id}"
    ...
    try:
        ...
        command = (
            f"cd '{remote_dir}' && '{args.python_executable}' exp003_lightgbm_cv.py --data-dir data --out-dir outputs "
