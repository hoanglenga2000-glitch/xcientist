...
def main() -> None:
    ...
    if set(ports) != required_ports:
        fail(
            "docker compose must publish workstation ports on loopback only",
            {"ports": ports, "required_ports": sorted(required_ports)},
        )
    if service.get("network_mode") == "host":
        fail("docker compose must not use host networking for the unauthenticated local workstation")
