#!/usr/bin/env python3
"""Download all 56 MLE-Bench 75 comps - simple version, CLI only."""
import subprocess, os, json, glob, zipfile, time

BASE = os.path.expanduser("~/jinghw/scripts/gpu_tra/mlebench_raw_data")
LOG = os.path.expanduser("~/jinghw/scripts/gpu_tra/download_v2.log")
RESULTS = os.path.expanduser("~/jinghw/scripts/gpu_tra/download_v2.json")
...
def try_dl(comp):
    ...
    for z in glob.glob(os.path.join(target, "*.zip")):
        try:
            ...
            os.remove(z)
            log(f"  Removed corrupt {os.path.basename(z)}")
