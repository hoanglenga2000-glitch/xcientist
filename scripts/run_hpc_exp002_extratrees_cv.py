...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run EXP002 ExtraTrees CV on the verified HPC SSH path.")
    ...
    remote_dir = f"{args.remote_root.rstrip('/')}/playground_series_s6e6/EXP002_{mode}_{run_id}"
    local_artifact_dir = ROOT / "workspace" / "hpc_experiments" / "playground_series_s6e6" / f"EXP002_{mode}_{run_id}"
    ...
    try:
        ...
        if all(remote_file_exists(sftp, f"{remote_existing_data}/{name}") for name in ["train.csv", "test.csv", "sample_submission.csv"]):
            copy_command = " && ".join(
                [
                    f"cp '{remote_existing_data}/{name}' '{remote_dir}/data/{name}'"
        ...
        upload_file(sftp, ROOT / "notebooks_or_scripts" / "exp002_extratrees_cv.py", f"{remote_dir}/exp002_extratrees_cv.py")
        command = (
            f"cd '{remote_dir}' && python exp002_extratrees_cv.py --data-dir data --out-dir outputs "
            f"--folds {args.folds} --seeds '{args.seeds}' --n-estimators {args.n_estimators} "
            f"--min-samples-leaf {args.min_samples_leaf} --max-features '{args.max_features}' "
