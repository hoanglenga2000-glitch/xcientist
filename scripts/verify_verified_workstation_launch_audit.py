...
def process_matches_runtime(pid: int, port: int, root: Path) -> bool:
    ...
    expected_standalone = (
        root / "web" / "research-agent-workstation" / ".next" / "standalone" / "server.js"
    ).resolve()
    expected_standalone_text = str(expected_standalone).replace("\\", "/").lower()
    port_pattern = re.compile(rf"(?:^|\s)--port(?:=|\s+)[\"']?{port}(?:[\"']?(?:\s|$))")
    return expected_standalone_text in normalized or (
        expected_cli_text in normalized and bool(port_pattern.search(normalized))
    )
...
def fetch_dashboard_health(dashboard_url: str, timeout: float = 5.0) -> dict[str, object] | None:
    ...
    try:
        request = Request(health_url, headers=authenticated_headers(dashboard_url))
...
def verify_runtime_binding(report: dict, root: Path) -> dict:
    ...
    require(
        runtime.get("mode") in {"start", "source-standalone"},
        "verified launch must use a production Next.js runtime",
        {"runtime": runtime},
    )
    require(
    ...
    require(
        runtime.get("source_build_stale") is False,
        "verified launch source build is stale",
        {"runtime": runtime},
    )
    require(
        runtime.get("dashboard_identity_verified") is True
        and runtime.get("runtime_identity_verified") is True,
        "verified launch lifecycle identity is not proven",
        {"runtime": runtime},
    )
    require(
    ...
    state_path = app_dir / ".runtime-logs" / "dashboard.process.json"
    ...
    require(state.get("schema") == "evomind.lifecycle_state.v2", "dashboard lifecycle state schema is invalid")
    dashboard_process = (state.get("processes") or {}).get("dashboard") or {}
    runtime_process = (state.get("processes") or {}).get("runtime") or {}
    require(dashboard_process.get("pid") == pid, "dashboard runtime PID changed after audit")
    require(dashboard_process.get("port") == port, "dashboard runtime port changed after audit")
    require(state.get("mode") == "source-standalone", "dashboard lifecycle is not source-standalone")
    require(
        runtime_process.get("pid") == runtime.get("runtime_pid")
        and runtime_process.get("port") == runtime.get("runtime_port"),
        "backend runtime identity changed after audit",
    )
...
def main() -> None:
    ...
    for key in ("openai", "deepseek", "claude", "kaggle", "hpc_ssh"):
        require(isinstance(dpapi.get(key), bool), f"{key} DPAPI audit flag must be boolean", {"dpapi_loaded": dpapi})
    require(
        dpapi.get("openai") is True or dpapi.get("deepseek") is True or dpapi.get("claude") is True,
    ...
    if dpapi.get("openai") is True:
        required_labels.add("openai_gateway_smoke")
    elif dpapi.get("deepseek") is True:
    ...
    failed = {
        ...
        if not item.get("ok") and not item.get("allow_failure")
    }
    require(not failed, "one or more verified launcher smoke steps failed", {"failed": failed})
    allowed_failures = {
        label: item
        for label, item in labels.items()
        if not item.get("ok") and item.get("allow_failure")
    }
    require(
        not allowed_failures or report.get("allow_resource_blockers") is True,
        "allowed resource failures require the explicit resource-blocker launch mode",
    ...
    openai_verified = bool(dpapi.get("openai") and labels.get("openai_gateway_smoke", {}).get("ok"))
    ...
    provider_verified = openai_verified or deepseek_verified or code_agent_verified
    require(
    ...
    if dpapi.get("claude"):
        require(
            ...
            or bool(allowed_failures.get("external_gateway_smoke")),
