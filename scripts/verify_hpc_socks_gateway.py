...
def run_windows_bridge_manager(action: str) -> dict[str, Any]:
    manager = ROOT / "scripts" / "manage_hpc_proxy_bridge.ps1"
    powershell = "powershell.exe" if os.name != "nt" else "powershell"
    completed = subprocess.run(
        [powershell, "-ExecutionPolicy", "Bypass", "-File", str(manager), action],
        text=True,
        capture_output=True,
        timeout=30,
    )
...
def verified_dpapi_bridge(dest_host: str, dest_port: int) -> dict[str, Any] | None:
    try:
        ...
        if status.get("returncode") != 0 or status.get("status") != "running" or not status.get("credential_installed"):
            return None
        test = run_windows_bridge_manager("test")
...
def main() -> None:
    ...
    parser.add_argument("--proxy-host", default=os.environ.get("GPU_SSH_SOCKS_HOST") or os.environ.get("HPC_SOCKS_HOST") or "8.163.52.223")
    parser.add_argument("--proxy-port", type=int, default=int(os.environ.get("GPU_SSH_SOCKS_PORT") or os.environ.get("HPC_SOCKS_PORT") or "1080"))
    parser.add_argument("--dest-host", default=os.environ.get("GPU_SSH_HOST") or "100.85.169.63")
    parser.add_argument("--dest-port", type=int, default=int(os.environ.get("GPU_SSH_PORT") or "1235"))
