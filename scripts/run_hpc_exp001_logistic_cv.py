...
def main() -> None:
    ...
    remote_dir = f"{args.remote_root.rstrip('/')}/playground_series_s6e6/EXP001_{run_id}"
    local_artifact_dir = ROOT / "workspace" / "hpc_experiments" / "playground_series_s6e6" / f"EXP001_{run_id}"
    ...
    try:
        ...
        if all(remote_file_exists(sftp, f"{remote_existing_data}/{name}") for name in ["train.csv", "test.csv", "sample_submission.csv"]):
            _, stdout, stderr = client.exec_command(f"cp '{remote_existing_data}/train.csv' '{remote_dir}/data/train.csv' && cp '{remote_existing_data}/test.csv' '{remote_dir}/data/test.csv' && cp '{remote_existing_data}/sample_submission.csv' '{remote_dir}/data/sample_submission.csv'", timeout=600)
        ...
        upload_file(sftp, ROOT / "notebooks_or_scripts" / "exp001_logistic_cv.py", f"{remote_dir}/exp001_logistic_cv.py")
        command = f"cd '{remote_dir}' && python exp001_logistic_cv.py --data-dir data --out-dir outputs --folds 5"
        started = time.time()
