...
$DataRoot = if ($BundleMode) { Join-Path $ManagedLocalRoot "data" } elseif ($env:WORKSTATION_DATA_DIR) { [IO.Path]::GetFullPath($env:WORKSTATION_DATA_DIR) } else { Join-Path $Root "user-data" }
