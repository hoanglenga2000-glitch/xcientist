#!/usr/bin/env python3
"""Safely reconstruct and verify a Spooky OOF run with legacy bundle support."""
...
BUNDLE_ID_ARRAYS = ("train_id", "test_id")
VERIFIER_VERSION = "legacy_object_id_safe_v2"
...
def load_prediction_bundle(
    ...
    required: set[str],
    *,
    id_array_names: Sequence[str] = BUNDLE_ID_ARRAYS,
) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    """Load numeric arrays without pickle and quarantine legacy object IDs."""
    arrays: dict[str, np.ndarray] = {}
    id_names = set(id_array_names)
    ...
    with np.load(path, allow_pickle=False) as archive:
        ...
        for name in sorted(required):
            try:
                ...
                if name not in id_names or "Object arrays cannot be loaded" not in str(exc):
                    raise
                diagnostics[name] = {
                    "loaded": False,
                    "safe_without_pickle": False,
                    "status": "legacy_object_dtype_rejected",
                    "error": str(exc),
                }
                continue
            arrays[name] = value
            if name in id_names:
                diagnostics[name] = {
                    "loaded": True,
                    "safe_without_pickle": True,
                    "status": "loaded_without_pickle",
    ...
    return arrays, {
        "allow_pickle": False,
        "bundle_id_arrays_safe": all(
            diagnostics[name]["safe_without_pickle"] for name in id_names
        ),
        "id_arrays": diagnostics,
    }


def id_sequence_diagnostic(values: Sequence[Any], *, source: Path) -> dict[str, Any]:
    """Describe public IDs without embedding the full sequence in the report."""
    ...
    return {
        ...
        "purpose": "public_expected_ids_diagnostic_only",
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    arrays, bundle_load = load_prediction_bundle(bundle_path, required)
    input_hashes = {
    ...
    expected_train_ids = train["id"].astype(str).to_numpy()
    expected_test_ids = test["id"].astype(str).to_numpy()
    identity_checks = {
        "truth": np.array_equal(arrays["truth"].astype(np.int64), truth),
        "train_id": "train_id" in arrays
        and np.array_equal(arrays["train_id"].astype(str), expected_train_ids),
        "test_id": "test_id" in arrays
        and np.array_equal(arrays["test_id"].astype(str), expected_test_ids),
    ...
    expected_id_diagnostics = {
        "train_id": id_sequence_diagnostic(expected_train_ids, source=train_path),
        "test_id": id_sequence_diagnostic(expected_test_ids, source=test_path),
    }
    component_oof = {
    ...
    contract_checks = {
        ...
        "bundle_id_arrays_safe": bundle_load["bundle_id_arrays_safe"],
    ...
    report = {
        ...
        "expected_id_diagnostics": expected_id_diagnostics,
