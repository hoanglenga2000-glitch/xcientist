...
def source_tree_digest(app_dir: Path = APP_DIR) -> str:
    ...
    for directory in SOURCE_DIRECTORIES:
        ...
        if root.is_dir():
            files.extend(path for path in root.rglob("*") if path.is_file() and path.suffix != ".db")
...
def start(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({"status": "started", "url": control_url_for(args.port), "health_url": url_for(args.port, "/api/workstation-summary"), **runtime_state, **ready}, ensure_ascii=False, indent=2))
...
def stop(args: argparse.Namespace) -> None:
    ...
    if stopped and PID_FILE.exists():
        PID_FILE.unlink()
    if stopped and STATE_FILE.exists():
        STATE_FILE.unlink()
