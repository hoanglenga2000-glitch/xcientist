...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run EXP006 CatBoost CV on the verified HPC SSH path.")
    ...
    remote_dir = f"{args.remote_root.rstrip('/')}/playground_series_s6e6/EXP006_{mode}_{run_id}"
    local_artifact_dir = ROOT / "workspace" / "hpc_experiments" / "playground_series_s6e6" / f"EXP006_{mode}_{run_id}"
    ...
    try:
        ...
        remote_existing_data = f"{args.remote_root.rstrip('/')}/playground_series_s6e6/20260614_183531/data"
        if all(remote_file_exists(sftp, f"{remote_existing_data}/{name}") for name in ["train.csv", "test.csv", "sample_submission.csv"]):
        ...
        upload_file(sftp, ROOT / "notebooks_or_scripts" / "exp006_catboost_cv.py", f"{remote_dir}/exp006_catboost_cv.py")
        command = (
            f"cd '{remote_dir}' && '{args.python_executable}' exp006_catboost_cv.py --data-dir data --out-dir outputs "
            f"--folds {args.folds} --seeds '{args.seeds}' --iterations {args.iterations} "
            f"--learning-rate {args.learning_rate} --sample-rows {args.sample_rows} "
            f"--task-type {args.task_type} --devices '{args.devices}'"
        )
        started = time.time()
