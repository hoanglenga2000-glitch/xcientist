#!/usr/bin/env python3
"""Deploy, monitor, and verify the frozen Taxi CPU precompute on job 89941."""
...
PLAN_SCHEMA = "evomind.mlebench.taxi_public_cache_plan.v1"
STATUS_SCHEMA = "evomind.hpc.job89941_taxi_public_cache_status.v1"
DEPLOYMENT_SCHEMA = "evomind.hpc.job89941_taxi_public_cache_deployment.v1"
COLLECTION_SCHEMA = "evomind.hpc.job89941_taxi_public_cache_collection.v1"
REMOTE_STATE_SCHEMA = "evomind.hpc.job89941_taxi_public_cache_state.v1"
EXPECTED_JOB_ID = 89941
EXPECTED_COMPETITION = "new-york-city-taxi-fare-prediction"
EXPECTED_SOURCE_PATHS = {
    "scripts/precompute_taxi_public_cache.py",
...
EXPECTED_INPUT_PATHS = {
    "prepared/public/labels.csv",
    "prepared/public/test.csv",
    "prepared/public/sample_submission.csv",
}
RUN_ID_PATTERN = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$")
SHA256_PATTERN = re.compile(r"^[0-9a-f]{64}$")
DEFAULT_PLAN_GLOB = "taxi_public_cache_job89941_frozen_plan_*.json"
DEFAULT_EVIDENCE_DIR = PROJECT_ROOT / "workspace" / "hpc" / "job89941_taxi_public_cache"
...
def build_runner_argv(plan: FrozenPlan, layout: RemoteLayout, *, verify_only: bool = False) -> list[str]:
    runner = ensure_remote_write_path(
        posixpath.join(layout.staged_root, "scripts", "precompute_taxi_public_cache.py")
    )
    argv = [
        "python3",
        runner,
        ...
        layout.root,
        "--max-rows",
        str(plan.payload["max_rows"]),
        "--chunk-rows",
        str(plan.payload["chunk_rows"]),
        "--cache-seed",
        str(plan.payload["seed"]),
        "--folds",
        str(plan.payload["folds"]),
        "--holdout-fraction",
        str(plan.payload["holdout_fraction"]),
    ]
    if verify_only:
...
print(json.dumps({{'schema':'evomind.hpc.job89941_taxi_public_cache_preflight.v1','root_ok':root_ok,'inputs':actual,'errors':errors,'private_paths_read':[],'passed':root_ok and not errors and len(actual)==3}}, sort_keys=True))
"""
...
print(json.dumps({{'schema':'evomind.hpc.job89941_taxi_public_cache_staging.v1','checks':checks,'passed':bool(checks) and all(x['matches'] for x in checks)}},sort_keys=True))
"""
    return _python_command(source)
...
import precompute_taxi_public_cache as runner
payload={'schema':'evomind.hpc.job89941_taxi_public_cache_import_smoke.v1','cuda_visible_devices':os.environ.get('CUDA_VISIBLE_DEVICES'),'cache_schema':full.TAXI_PUBLIC_CACHE_SCHEMA,'runner':runner.__file__,'passed':os.environ.get('CUDA_VISIBLE_DEVICES')==''}
...
if pid>0 and pathlib.Path(f'/proc/{{pid}}').is_dir():
    ...
    running={posixpath.join(layout.staged_root, 'scripts', 'precompute_taxi_public_cache.py')!r} in cmdline and {layout.cache!r} in cmdline
