#!/usr/bin/env python3
"""Evaluate a PUBLIC_ONLY cross-fitted Taxi airport expert successor on CPU."""
...
RESULT_SCHEMA = "evomind.mlebench.taxi_cpu_airport_expert_diagnostic.v1"
...
def airport_mask(
    ...
    index = feature_indices(feature_names, ("airport_trip",))["airport_trip"]
    values = np.asarray(features)
    if values.ndim != 2 or values.shape[1] != len(feature_names):
        raise TaxiLonghaulDiagnosticError("Taxi feature matrix shape changed")
    mask = np.asarray(values[:, index], dtype=np.float64) >= 0.5
    if not mask.any() or mask.all():
        raise TaxiLonghaulDiagnosticError("Airport mask is degenerate")
    return mask
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    selected = airport_mask(train_features, feature_names)
    ...
    for fold in sorted(int(value) for value in np.unique(cache_folds)):
        ...
        if len(fit) < 10_000 or len(valid) < 1_000:
            raise TaxiLonghaulDiagnosticError("Airport fold is too small")
        ...
        fold_records.append(
            {
                "fold": fold,
                "fit_airport_rows": len(fit),
                "valid_airport_rows": len(valid),
                "incumbent_airport_rmse": rmse(target[valid], incumbent[valid]),
                "expert_airport_rmse": rmse(target[valid], prediction),
                "model": file_record(model_path),
            }
        )
        del fit_x, valid_x, model
    if not np.isfinite(expert_oof[selected]).all():
        raise TaxiLonghaulDiagnosticError("Airport expert OOF coverage is incomplete")
    ...
    incumbent_airport_rmse = rmse(target[selected], incumbent[selected])
    expert_airport_rmse = rmse(target[selected], expert_oof[selected])
    passed = (
        successor_rmse <= args.maximum_successor_rmse
        and successor_rmse <= incumbent_rmse - args.minimum_overall_improvement
        and expert_airport_rmse
        <= incumbent_airport_rmse
        * (1.0 - args.minimum_airport_relative_improvement)
    )
    bundle_output = output_dir / "airport_expert_oof.npz"
    np.savez_compressed(
        bundle_output,
        ...
        airport_mask=selected.astype(np.uint8),
    ...
    report = {
        ...
        "model_family": "LightGBM_CPU_cross_fitted_airport_successor",
        "seed": args.seed,
        "folds": int(cache["folds"]),
        "iterations": args.iterations,
        "threads": args.threads,
        "airport_rows": int(selected.sum()),
        "airport_fraction": float(selected.mean()),
        "metrics": {
            ...
            "incumbent_airport_rmse": incumbent_airport_rmse,
            "expert_airport_rmse": expert_airport_rmse,
            "airport_relative_improvement": (
                (incumbent_airport_rmse - expert_airport_rmse)
                / incumbent_airport_rmse
            ),
        },
        "diagnostic_gate": {
            ...
            "minimum_airport_relative_improvement": (
                args.minimum_airport_relative_improvement
            ),
        },
        ...
        "claim_boundary": (
            "Cross-fitted public OOF airport diagnostic only; "
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--minimum-airport-relative-improvement",
        type=float,
        default=0.05,
    )
