#!/usr/bin/env python3
"""Evaluate a PUBLIC_ONLY cross-fitted Taxi short-haul successor on CPU."""
...
RESULT_SCHEMA = "evomind.mlebench.taxi_cpu_shorthaul_expert_diagnostic.v1"
...
def shorthaul_mask(
    ...
    *,
    ...
    if maximum_distance_km <= 0:
        raise TaxiLonghaulDiagnosticError("Maximum short-haul distance must be positive")
    ...
    mask = (distance < maximum_distance_km) & (airport < 0.5)
    if not mask.any() or mask.all():
        raise TaxiLonghaulDiagnosticError("Short-haul mask is degenerate")
    return mask
...
def validate_inputs(
    ...
    expected_cache = predecessor.get("base_cache_manifest") or {}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for fold in sorted(int(value) for value in np.unique(cache_folds)):
        ...
        if len(fit) < 10_000 or len(valid) < 1_000:
            raise TaxiLonghaulDiagnosticError("Short-haul fold is too small")
        ...
        fold_records.append(
            {
                "fold": fold,
                "fit_shorthaul_rows": len(fit),
                "valid_shorthaul_rows": len(valid),
                "incumbent_shorthaul_rmse": rmse(target[valid], incumbent[valid]),
                "expert_shorthaul_rmse": rmse(target[valid], prediction),
                "model": file_record(model_path),
            }
        )
        del fit_x, valid_x, model
    if not np.isfinite(expert_oof[selected]).all():
        raise TaxiLonghaulDiagnosticError("Short-haul expert OOF coverage is incomplete")
    ...
    incumbent_short_rmse = rmse(target[selected], incumbent[selected])
    expert_short_rmse = rmse(target[selected], expert_oof[selected])
    passed = (
        successor_rmse <= incumbent_rmse - args.minimum_overall_improvement
        and expert_short_rmse
        <= incumbent_short_rmse
        * (1.0 - args.minimum_shorthaul_relative_improvement)
    )
    bundle_output = output_dir / "shorthaul_expert_oof.npz"
    np.savez_compressed(
        bundle_output,
        ...
        shorthaul_mask=selected.astype(np.uint8),
    ...
    report = {
        ...
        "model_family": "LightGBM_CPU_cross_fitted_shorthaul_successor",
        "seed": args.seed,
        "folds": int(cache["folds"]),
        "iterations": args.iterations,
        "threads": args.threads,
        "maximum_distance_km": args.maximum_distance_km,
        "shorthaul_rows": int(selected.sum()),
        "shorthaul_fraction": float(selected.mean()),
        "metrics": {
            ...
            "incumbent_shorthaul_rmse": incumbent_short_rmse,
            "expert_shorthaul_rmse": expert_short_rmse,
            "shorthaul_relative_improvement": (
                (incumbent_short_rmse - expert_short_rmse)
                / incumbent_short_rmse
            ),
        },
        "diagnostic_gate": {
            ...
            "minimum_shorthaul_relative_improvement": (
                args.minimum_shorthaul_relative_improvement
            ),
        },
        ...
        "claim_boundary": (
            "Cross-fitted public OOF successor diagnostic only; "
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--maximum-distance-km", type=float, default=1.0)
    parser.add_argument("--minimum-overall-improvement", type=float, default=0.02)
    parser.add_argument(
        "--minimum-shorthaul-relative-improvement",
        type=float,
        default=0.05,
    )
