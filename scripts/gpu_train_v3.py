#!/usr/bin/env python3
"""
GPU V4: CatBoost training with cat_features support + time-series CV.
- CatBoost native categorical handling (no high-cardinality dropping)
...
- --no-log1p flag to disable log1p target transform
"""
...
HOME = '/hpc2hdd/home/aimslab'  # Kaggle data parent dir
WORK = '/hpc2hdd/home/aimslab/jinghw'  # My workspace for results/logs

# Competitions where random KFold causes data leakage → use TimeSeriesSplit
TIME_SERIES_COMPETITIONS = {"store_sales", "bike_sharing_demand"}
...
def bike_sharing_features(train, test):
    # Drop leak columns: casual+registered exist in train but NOT test.
    # Model learns count ≈ casual+registered, then fails on test (NaN→median→constant).
...
def store_sales_features(train, test):
    for df in [df for df in [train, test] if df is not None]:
        if 'date' in df.columns:
            ...
            df['day_of_year'] = dt.dt.dayofyear.astype(int)
        if 'onpromotion' in df.columns:
            df['onpromotion'] = df['onpromotion'].fillna(0).astype(int)
        if 'family' in df.columns and 'store_nbr' in df.columns:
            df['store_family'] = df['store_nbr'].astype(str) + '_' + df['family'].astype(str)
        if 'item_nbr' in df.columns and 'store_nbr' in df.columns:
            df['store_item'] = df['store_nbr'].astype(str) + '_' + df['item_nbr'].astype(str)
    return train, test

FEATURE_HOOKS = {
    "titanic": titanic_features,
    "spaceship_titanic": spaceship_features,
    "bike_sharing_demand": bike_sharing_features,
    "store_sales": store_sales_features,
}
...
def load_and_preprocess(task_id):
    ...
    # Get submission format early for ID detection
    ...
    # Identify ID columns - auto-detect + submission format
    auto_id = [c for c in train.columns if c.lower().replace(' ','_') in ('id','passengerid','imageid')]
    if id_col_out and id_col_out not in auto_id and id_col_out in train.columns:
        auto_id.insert(0, id_col_out)
    id_cols = auto_id
    ...
    if test is not None and id_cols:
        tid = id_cols[0]
        if tid in test.columns:
            test_ids = test[tid].values.copy()
        # For datetime IDs, read from test.csv directly
        elif test_path and os.path.exists(test_path):
            test_raw = pd.read_csv(test_path)
            if tid in test_raw.columns:
                test_ids = test_raw[tid].values.copy()
    ...
    # Combine and encode — keep high-cardinality categoricals for CatBoost
    combined = pd.concat([train_feat, test_feat] if test_feat is not None else [train_feat], ignore_index=True)
    cat_features = []  # column indices for CatBoost native categorical handling
    high_card_cols = set()  # track columns that were originally object with >50 unique values
        ...
        if combined[col].dtype == 'object':
            ...
            if n_unique == 1:
                ...
                if n_unique <= 50:
                    ...
                    # High-cardinality: track for CatBoost cat_features
                    combined[col] = combined[col].fillna('MISSING')
                    combined[col] = LabelEncoder().fit_transform(combined[col].astype(str))
                    high_card_cols.add(col)
        elif combined[col].dtype in ('float64', 'int64'):
            combined[col] = combined[col].fillna(combined[col].median())
        elif combined[col].dtype == 'bool':
            combined[col] = combined[col].astype(int)

    # Build cat_features list from originally-high-cardinality columns
    for col in high_card_cols:
        if col in combined.columns:
            col_idx = combined.columns.get_loc(col)
    ...
    print(f"  Features: {X_train.shape[1]} total, {len(cat_features)} cat_features")

    # Set fallback submission format if not already set
    if id_col_out is None:
        id_col_out = id_cols[0] if id_cols else 'id'
        pred_col_out = target
        val_fmt = 'int'

    return X_train, y, X_test, task_type, metric, direction, bronze, margin, test_ids, id_col_out, pred_col_out, val_fmt, target_encoder, cat_features
...
def train_catboost(X_train, y, X_test, task_type, metric, gpu_device, n_folds, fast, depth, l2, lr, n_iter_override, cat_features, task_id, no_log1p=False):
    n_iter = n_iter_override if n_iter_override > 0 else (300 if fast else 1000)
    early_stop = 30 if fast else 80
    log1p_target = (metric == "rmsle") and not no_log1p

    if task_type in ('binary', 'multiclass'):
        folds = list(StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=42).split(X_train, y))
    else:
        # NOTE: TimeSeriesSplit skips first test_size samples (never in any val fold),
        # causing OOF zeros and massively inflated global metric. Use standard KFold.
    ...
    # Detect extreme imbalance for binary classification
    ...
    if task_type == 'binary':
        ...
        if imbalance_ratio > 3:
            ...
            print(f"  Imbalance detected: {n_neg} neg / {n_pos} pos = {imbalance_ratio:.1f}:1 → scale_pos_weight={scale_pos_weight:.1f}")

    cb_params = {
        'iterations': n_iter,
        'learning_rate': lr if lr > 0 else 0.02,
        'depth': depth if depth > 0 else 5,
        'l2_leaf_reg': l2 if l2 > 0 else 5,
    ...
    if task_type == 'binary' and scale_pos_weight > 1.0:
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(folds):
        ...
        # log1p transform for RMSLE tasks
        y_val_orig = y_val.copy()
        if log1p_target:
            y_tr = np.log1p(np.maximum(y_tr, 0))
            y_val = np.log1p(np.maximum(y_val, 0))
        ...
        if task_type == 'multiclass':
            ...
            fold_score = compute_oof_metric(y_val_orig, val_pred, metric, task_type)
        elif task_type == 'binary':
    ...
    # Debug: verify OOF assembly
    oof_global = compute_oof_metric(y, oof, metric, task_type)
    oof_per_fold_mean = float(np.mean(scores))
    print(f"  DEBUG OOF: per_fold_mean={oof_per_fold_mean:.4f}, global={oof_global:.4f}, ratio={oof_global/max(oof_per_fold_mean,0.001):.1f}x")
    # Check for zero/negative predictions in OOF
    n_zero = int((oof == 0).sum()) if task_type == 'regression' else 0
    n_neg = int((oof < 0).sum()) if task_type == 'regression' else 0
    if n_zero > 0 or n_neg > 0:
        print(f"  DEBUG OOF: {n_zero} zero preds, {n_neg} negative preds")
    # Spot-check alignment: compare y vs oof at quartile positions
    if len(oof) > 10:
        for i in [0, len(oof)//4, len(oof)//2, 3*len(oof)//4, len(oof)-1]:
            yi = y.iloc[i]
            oi = oof[i]
            print(f"  DEBUG align[{i}]: y={yi:.2f}, oof={oi:.2f}, ratio={oi/max(abs(yi),0.1):.2f}")
...
def main():
    ...
    parser.add_argument('--no-log1p', action='store_true', help='Disable log1p target transform (use for temporal regression)')
    ...
    X_train, y, X_test, task_type, metric, direction, bronze, margin, test_ids, id_col_out, pred_col_out, val_fmt, target_encoder, cat_features = result
    ...
    oof, test_preds, cv_scores = train_catboost(X_train, y, X_test, task_type, metric, args.gpu_device, n_folds, args.fast, args.depth, args.l2, args.lr, args.iter, cat_features, tid, args.no_log1p)
    ...
    sub_path = f"{WORK}/results/v3_submission_{tid}.csv"
    ...
    result_path = f"{WORK}/results/v3_result_{tid}.json"
