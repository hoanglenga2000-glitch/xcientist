...
def load_and_preprocess(task_id):
    ...
    # FIX: Drop NaN targets
    nan_mask = y.isna()
    if nan_mask.any():
        print(f"  [FIX] Dropping {nan_mask.sum()} rows with NaN target")
        train = train[~nan_mask.values].reset_index(drop=True)
        y = y[~nan_mask.values].reset_index(drop=True)

    # FIX: Binary tasks with >2 unique values -> multiclass
    if task_type == 'binary' and y.nunique() > 2:
        print(f"  [FIX] Binary task has {y.nunique()} unique targets, treating as multiclass")
...
def train_catboost(X_train, y, X_test, task_type, gpu_device, n_folds, fast, n_iter_override=None):
    n_iter = n_iter_override if n_iter_override else (300 if fast else 1000)
    ...
    if task_type in ('binary', 'multiclass'):
        try:
            ...
            print(f"  [FIX] StratifiedKFold failed, falling back to KFold")
...
def main():
    ...
    n_iter_override = args.n_iter if args.n_iter > 0 else None
    oof, test_preds, cv_scores = train_catboost(X_train, y, X_test, task_type, args.gpu_device, n_folds, args.fast, n_iter_override)
