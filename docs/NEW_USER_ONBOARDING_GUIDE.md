...
> 进阶：查看已存凭据状态用 `powershell -File scripts\manage_hpc_ssh_secret.ps1 status`；仅更新连接元数据（host/port/workspace）用 `... set-metadata`。凭据写入使用 `install-credential -User <用户名>`，口令由隐藏提示读取；不要把口令放入命令行参数。
