...
def test_jsonl_event_sink_writes_verifiable_events(tmp_path):
    ...
    from research_os.events import PROMOTE, RUN_BEGIN, RUN_END, SCORE, JsonlEventSink
...
def test_preflight_stages_render_all_six():
    ...
    stages = [
        ...
        ("Selecting compute", "compute=gpu", "passed"),
        ("Planning experiment", "goal=improve baseline CV", "passed"),
        ("Entering workstation agent", "compute=gpu, events → events.jsonl", "passed"),
    ]
