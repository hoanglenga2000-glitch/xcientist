...
def test_dashboard_post_rejects_non_http_urls(monkeypatch):
    monkeypatch.setattr(
        engineering.urllib.request,
        "urlopen",
        lambda *_args, **_kwargs: pytest.fail("network must not be reached"),
    )

    with pytest.raises(ValueError, match=r"http\(s\)"):
        engineering._post_json("file:///tmp/secret", {"task": "x"}, 1)


def test_dashboard_post_rejects_remote_cleartext_and_embedded_credentials(monkeypatch):
    monkeypatch.setattr(
        engineering.urllib.request,
        "urlopen",
        lambda *_args, **_kwargs: pytest.fail("network must not be reached"),
    ...
    with pytest.raises(ValueError, match="require https"):
        engineering._post_json("http://example.invalid/api/tasks", {"task": "x"}, 1)
    with pytest.raises(ValueError, match="embedded credentials"):
        engineering._post_json("https://user:password@example.invalid/api/tasks", {"task": "x"}, 1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "http://127.0.0.1:8088/api/tasks",
        "http://[::1]:8088/api/tasks",
        "https://example.invalid/api/tasks",
    ],
)
def test_dashboard_post_allows_https_and_loopback_http(monkeypatch, url):
    class Response:
        def __enter__(self):
            return self
    ...
    monkeypatch.setattr(engineering.urllib.request, "urlopen", lambda *_args, **_kwargs: Response())

    assert engineering._post_json(url, {"task": "x"}, 1) == {}
