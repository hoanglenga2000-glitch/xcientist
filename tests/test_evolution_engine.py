...
from research_os.evolution_loop import (
    LocalSubprocessRunner as ReleaseLocalSubprocessRunner,
)
from research_os.hpc_policy import HPCPolicyError
from research_os.llm_client import LLMClient, LLMError, LLMResponse, ProviderConfig, _env, _post_json
...
def test_provider_config_repr_hides_api_key():
    config = ProviderConfig("anthropic", "https://example.invalid", "model", "sentinel-secret")

    assert "sentinel-secret" not in repr(config)
    assert "sentinel-secret" not in str(config)
...
def test_llm_client_resolves_openai_as_distinct_provider(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "fixture-openai-key")
    monkeypatch.setenv("OPENAI_BASE_URL", "https://example.invalid")
    monkeypatch.setenv("OPENAI_MODEL", "fixture-openai-model")
    ...
    assert config.model == "fixture-openai-model"
    assert config.api_key == "fixture-openai-key"


def test_llm_http_client_rejects_non_http_urls(monkeypatch):
    monkeypatch.setattr(
        "research_os.llm_client.urllib.request.urlopen",
        lambda *_args, **_kwargs: pytest.fail("network must not be reached"),
    )

    with pytest.raises(ValueError, match=r"http\(s\)"):
        _post_json("file:///tmp/secret", {}, {"prompt": "x"}, 1)


def test_llm_http_client_rejects_remote_cleartext_and_embedded_credentials(monkeypatch):
    monkeypatch.setattr(
        "research_os.llm_client.urllib.request.urlopen",
        lambda *_args, **_kwargs: pytest.fail("network must not be reached"),
    )
    ...
    with pytest.raises(ValueError, match="require https"):
        _post_json("http://example.invalid/v1/messages", {}, {"prompt": "x"}, 1)
    with pytest.raises(ValueError, match="embedded credentials"):
        _post_json("https://user:password@example.invalid/v1/messages", {}, {"prompt": "x"}, 1)
...
def test_llm_http_client_allows_https_and_loopback_http(monkeypatch, url):
    class Response:
        def __enter__(self):
            return self
    ...
    monkeypatch.setattr("research_os.llm_client.urllib.request.urlopen", lambda *_args, **_kwargs: Response())

    assert _post_json(url, {}, {"prompt": "x"}, 1) == {}
...
def test_gpu_runner_does_not_fabricate_score_from_stale_metrics_on_kill():
    ...
    runner = GPURunner(
        "essay",
        config=GPURunnerConfig(remote_root="/remote/evomind", timeout=60),
        connect=lambda: ssh,
    )
...
def test_release_local_runner_fails_closed_without_filesystem_side_effects(tmp_path):
    runner = ReleaseLocalSubprocessRunner(tmp_path / "work", timeout=60)
