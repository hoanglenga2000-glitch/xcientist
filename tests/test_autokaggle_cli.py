...
from xsci.agentic_capability_benchmark import build_benchmark_cases
...
def test_official_console_entry_reads_process_arguments(monkeypatch):
    calls = []
    process_argv = ["kaggle-official", "datasets", "list"]
    fake_pkg = types.ModuleType("kaggle")
    fake_cli = types.ModuleType("kaggle.cli")
    ...
    assert rc == 0
    assert calls == [["kaggle", "datasets", "list"]]
...
def test_session_marks_configured_gpu_blocked_by_external_manifest(isolated_autokaggle):
    ...
    xcfg.set_global("gpu_ssh", "user", "researcher-test")
...
def test_local_compute_override_cannot_bypass_gpu_manifest_blocker(isolated_autokaggle, monkeypatch, capsys):
    root = xcfg.active_root()
    manifest = Path.cwd() / "configs" / "external_resources.yaml"
    manifest.parent.mkdir(parents=True, exist_ok=True)
    manifest.write_text(
    ...
    assert not should_exit
    assert not calls
    assert not (root / ".xsci" / "scientist_decision.json").exists()
...
def test_run_setup_records_openai_as_openai_not_deepseek(isolated_autokaggle, monkeypatch):
    answers = iter(["3"])
    monkeypatch.setattr(ak, "_safe_input", lambda prompt, default="": next(answers, default))
    monkeypatch.setattr(ak, "_yes", lambda *args, **kwargs: False)
    monkeypatch.setattr(ak.getpass, "getpass", lambda *args, **kwargs: "fixture-openai-key")

    assert ak.run_setup(force=True) == 0

    cfg = xcfg.load_config()
    assert cfg.get("llm.provider") == "openai"
    assert cfg.get("secrets.openai_api_key") == "fixture-openai-key"
    assert cfg.get("secrets.deepseek_api_key") is None
    assert cfg.get("compute.backend") == "gpu"
...
def test_openai_config_exports_distinct_key_base_url_and_model(monkeypatch):
    fake_env: dict = {}
    monkeypatch.setattr(xcfg.os, "environ", fake_env)
    cfg = xcfg.Config(data={
        "llm": {
            "provider": "openai",
            "model": "gpt-test",
            "openai_base_url": "https://api.openai.example",
        },
        "secrets": {"openai_api_key": "fixture-openai-key"},
    })

    injected = xcfg.inject_engine_env(cfg, override=True)

    assert fake_env["OPENAI_API_KEY"] == "fixture-openai-key"
    assert fake_env["OPENAI_BASE_URL"] == "https://api.openai.example"
    assert fake_env["OPENAI_MODEL"] == "gpt-test"
    assert fake_env["EVOLUTION_PRIMARY_PROVIDER"] == "openai"
    assert "DEEPSEEK_API_KEY" not in fake_env
    assert set(injected) >= {"OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENAI_MODEL"}
...
def test_resume_intent_fails_closed_without_hpc(isolated_autokaggle, monkeypatch, capsys):
    """Resume intent reaches execution preflight but fails closed without HPC."""
    root = xcfg.active_root()
    xcfg.write_secret("anthropic_api_key", "sk-TEST")
    ak._register_task("https://www.kaggle.com/competitions/spaceship-titanic", root)
    calls = {}
...
def test_switch_task_then_local_train_fails_closed(isolated_autokaggle, monkeypatch):
    """Switch first, then keep the requested local training fail-closed."""
    root = xcfg.active_root()
    xcfg.write_secret("anthropic_api_key", "sk-TEST")
    ak._register_task("https://www.kaggle.com/competitions/spaceship-titanic", root)
    ak._register_task("https://www.kaggle.com/competitions/house-prices", root)
...
def test_scientist_checkpoint_tool_summarizes_state_without_secret(isolated_autokaggle):
    ...
    assert result["gate"]["planning_can_proceed"] is True
    assert result["gate"]["planning_blockers"] == []
    assert any("GPU/SSH" in blocker for blocker in result["gate"]["blockers"])
...
def test_scientist_autopilot_runs_multi_tool_chain_without_secret(isolated_autokaggle):
    ...
    assert "innovation_approval_command" not in blueprint
    assert run_action["command"] == "evomind run spaceship-titanic"
    assert run_action["gate"] == "hpc_runtime_and_human_run_command_required"
    assert run_action["autonomy"] == "requires_user_run_command"
    assert run_action["metadata"]["selected_hypothesis"]
    assert run_action["metadata"]["experiment_blueprint"]["blueprint_id"] == blueprint["blueprint_id"]
    assert run_action["metadata"]["hpc_gate_clear"] is False
...
def test_scientist_next_action_advances_read_only_while_training_blocked(isolated_autokaggle, monkeypatch, capsys):
    from xsci.terminal_tools import TerminalTools
    ...
    assert result["selected_action"]["gate"] == "setup_or_data_gate"
...
def test_scientist_loop_runs_bounded_safe_cycle_and_records_lesson(isolated_autokaggle, monkeypatch, capsys):
    ...
    assert result["final_next_action"]["selected_action"]["id"] == "clear_blockers"
    ...
    assert "read-only action" in result["lesson"]["lesson"]
    ...
    phases = [event.get("phase") for event in load_recent_scientist_step_events(root, limit=500)]
...
_AGENTIC_CASES = build_benchmark_cases()
_AGENTIC_CASE_CATEGORIES = {case.case_id: case.category for case in _AGENTIC_CASES}
_AGENTIC_RUNTIME_CONTRACT = {
    "benchmark_version": 2,
    "case_categories": _AGENTIC_CASE_CATEGORIES,
    "runtime_package_version": "0.2.2",
    "runtime_source_digest_sha256": "a" * 64,
    "runtime_source_file_count": 145,
    "runtime_git_commit": "b" * 40,
    "runtime_git_dirty": True,
}


def _agentic_behavior_report(*, passed_cases: int = 9, case_count: int = 12, issue: str = "") -> dict:
    selected = _AGENTIC_CASES[:case_count]
    results = []
    for index, case in enumerate(selected):
        passed = index < passed_cases
        results.append({
            ...
            "failure_reason": "" if passed else "hidden oracle did not pass",
            "duration_seconds": 1.0,
            "runner_output": json.dumps({
                "provider": "deepseek",
                "model": "deepseek-test",
                "worker_exit_code": 0,
                "worker_status": "completed" if passed else "needs_continuation",
            }),
        })
    if issue and results:
        target = results[0]
        target["passed"] = False
        target["oracle_passed"] = False
        target["failure_reason"] = issue
        if issue == "scope":
            target["scope_violation"] = True
            target["scope_violation_paths"] = ["outside.txt"]
    ...
    scope = sum(item["scope_violation"] is True for item in results)
    unsupported = sum(item["unsupported_claim"] is True for item in results)
    timed_out = sum(item["timed_out"] is True for item in results)
    return {
        "benchmark": "evomind_agentic_capability",
        "benchmark_version": 2,
        "adapter": "workspace_agent_subprocess_v1",
        "run_id": "agentic_benchmark_" + "c" * 32,
        "seed": 20260711,
        "generated_at": "2026-07-12T00:00:00+00:00",
        "provider": "deepseek",
        "selected_case_ids": [case.case_id for case in selected],
        "total_cases": len(_AGENTIC_CASES),
        ...
        "duration_seconds": float(len(results)),
        "case_results": results,
        "report_written": True,
        **{key: value for key, value in _AGENTIC_RUNTIME_CONTRACT.items() if key != "case_categories"},
    }
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "benchmark_version": 1},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "execution_status": "blocked"},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "provider": ""},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            _agentic_behavior_report(passed_cases=4, case_count=6),
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            _agentic_behavior_report(issue="scope"),
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            _agentic_behavior_report(issue="unsupported"),
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            _agentic_behavior_report(issue="timeout"),
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "runtime_package_version": "0.2.1"},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "runtime_source_digest_sha256": "d" * 64},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "runtime_git_commit": "e" * 40},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "runtime_git_dirty": False},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            {**_agentic_behavior_report(), "passed_cases": 12},
            False, 0, 59, "untrusted_agentic_benchmark_report",
        ),
        (
            _agentic_behavior_report(passed_cases=7),
            True, 12, 59, "insufficient_agentic_benchmark_performance",
        ),
        (
            _agentic_behavior_report(passed_cases=8),
            True, 12, 69, "partial_agentic_benchmark_evidence",
        ),
        (
            _agentic_behavior_report(passed_cases=9),
            True, 12, 84, "offline_agentic_benchmark_passed",
        ),
    ],
)
def test_scientist_self_audit_only_counts_completed_production_agent_benchmark(
    ...
    from xsci import terminal_tools as terminal_tools_module
    from xsci.terminal_tools import TerminalTools

    monkeypatch.setattr(
        terminal_tools_module,
        "_agentic_benchmark_runtime_contract",
        lambda: dict(_AGENTIC_RUNTIME_CONTRACT),
    )
    ...
    assert bool(behavior["trust_failures"]) is (not trusted)
    assert result["behavior_score_cap"] == expected_cap
    assert result["overall_score"] <= expected_cap


def test_scientist_self_audit_rejects_malformed_benchmark_summary_without_crashing(
    isolated_autokaggle,
    monkeypatch,
):
    from xsci import terminal_tools as terminal_tools_module
    ...
    report = _agentic_behavior_report()
    report["cases_run"] = "twelve"
    report["task_success_rate"] = "one"
    root = xcfg.active_root()
    xsci_dir = root / ".xsci"
    xsci_dir.mkdir(parents=True, exist_ok=True)
    (xsci_dir / "agentic_capability_benchmark.json").write_text(
    ...
    result = TerminalTools.dispatch("scientist_self_audit", SessionState.from_root(root), root)
    behavior = result["evidence_sources"]["agentic_behavior_benchmark"]

    assert behavior["trusted_evidence"] is False
    assert behavior["reported_cases_run"] == 0
    assert behavior["reported_task_success_rate"] == 0.0
    assert "incomplete_case_count" in behavior["trust_failures"]
    assert "task_success_rate_mismatch" in behavior["trust_failures"]
...
def test_scientist_two_pass_executes_requested_tool(isolated_autokaggle, monkeypatch):
    from xsci import kaggle_conversation as conversation

    root = xcfg.active_root()
    ak._register_task("https://www.kaggle.com/competitions/spaceship-titanic", root)
    state = SessionState.from_root(root)
    responses = iter([
        types.SimpleNamespace(text="Check the current model.\n[tool: model_status]"),
        types.SimpleNamespace(text="The model status is verified."),
    ])
    ...
    executed = []
    agent = conversation.ConversationAgent(client=FakeClient())
    monkeypatch.setattr(agent, "_real_tool_loop", lambda *args, **kwargs: "")
    monkeypatch.setattr(
        conversation,
        "_execute_terminal_tool",
        lambda name, session: executed.append(name) or "model_status: ready",
    )

    result = agent._scientist_loop(state, "Assess the available evidence.", [])

    assert result == "The model status is verified."
    assert executed == ["model_status"]
...
def test_scientist_execution_contract_allows_planning_but_blocks_training_without_hpc(isolated_autokaggle):
    from xsci.scientist_trace import load_recent_scientist_step_events
    from xsci.scientist_turns import load_recent_scientist_turns
    from xsci.terminal_tools import TerminalTools
    ...
    assert result["planning_can_proceed"] is True
    assert result["data_contract_status"] == "planning_ready_hpc_execution_blocked"
    assert result["execution_gate_decision"]["status"] == "blocked"
    assert result["execution_gate_decision"]["blocked"] is True
    assert "gpu_blocked" in result["root_causes"]
    assert any("GPU/SSH" in blocker for blocker in result["setup_blockers"])
    assert result["no_training_started"] is True
    assert result["official_submit"] == "blocked_until_explicit_human_approval"
...
def test_auto_mode_persists_research_decision_before_hpc_block(isolated_autokaggle, monkeypatch, capsys):
    root = xcfg.active_root()
    xcfg.write_secret("anthropic_api_key", "sk-TEST")
    ak._register_task("https://www.kaggle.com/competitions/spaceship-titanic", root)
    data_dir = root / "data" / "spaceship-titanic"
