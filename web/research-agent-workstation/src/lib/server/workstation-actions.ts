...
export async function handleWorkstationAction(payload: WorkstationActionPayload) {
  ...
  switch (action) {
    ...
    case "select_compute_mode": {
      ...
      const executionMode = requestedMode === "local" ? "local" : "hpc_gpu";
      const smallTaskOnly = payload.metadata?.small_task_only !== false;
      const computeSetting = {
        execution_mode: executionMode,
        local_training_enabled: executionMode === "local",
        local_training_scope: executionMode === "local" && smallTaskOnly ? "small_tasks_only" : "hpc_gpu_default",
        selected_at: new Date().toISOString(),
        selected_by: "workstation_gpu_page",
        note: executionMode === "local"
          ? "Local compute is explicitly selected for small controlled workstation tests. Official Kaggle submission remains human-gated."
          : "HPC/GPU cluster mode is selected. Local training fallback is disabled unless local mode is explicitly re-selected."
      };
      ...
      const artifact = await writeJsonArtifact(`workspace/settings/compute_mode_${stamp()}.json`, {
        ...
        local_training_policy: {
          allowed_now: executionMode === "local",
      ...
      const record = await logAction({
        action,
        taskId,
        message: executionMode === "local"
          ? "Compute mode switched to local for small controlled workstation training tests."
          : "Compute mode switched to HPC/GPU; local training fallback is disabled.",
        artifactPath: artifact,
        metadata: { execution_mode: executionMode, local_training_enabled: executionMode === "local" }
      });
    ...
    case "rotate_credentials_batch": {
      ...
      const artifact = await writeJsonArtifact(`workspace/settings/rotate_credentials_request_${stamp()}.json`, {
        ...
        next_safe_commands: [
          "scripts/manage_kaggle_secret.ps1 install-token -ApiToken <token>",
          "scripts/manage_hpc_ssh_secret.ps1 install-password -Host <host> -Port <port> -User <user>",
          "scripts/manage_deepseek_secret.ps1 install -ApiKey <key>"
        ],
        created_at: new Date().toISOString()
      });
      const record = await logAction({
