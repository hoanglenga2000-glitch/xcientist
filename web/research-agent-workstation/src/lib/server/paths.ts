...
export function toRelativePath(targetPath: string | null | undefined) {
  ...
  const target = path.isAbsolute(targetPath) ? path.resolve(targetPath) : resolveWorkspacePath(targetPath);
...
export function resolveWorkspacePath(relativePath: string) {
  ...
  if (path.isAbsolute(relative) || relative === ".." || relative.startsWith(`..${path.sep}`)) {
    throw new Error("Workspace path escapes the workstation root");
  }
  return target;
}
