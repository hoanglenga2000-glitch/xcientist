...
export async function runDeepSeekSmoke(prompt = "Return exactly: deepseek-ok"): Promise<DeepSeekSmokeResult> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(typeof payload.error === "object" ? JSON.stringify(payload.error) : `DeepSeek request failed: ${response.status}`);
    }
    ...
    const result: DeepSeekSmokeResult = {
      ...
      configured: true,
      status: "failed",
      error: error instanceof Error ? error.message : "DeepSeek smoke test failed.",
      missing_env: undefined
    };
    const artifact = await writeSmokeArtifact(result);
    await logAction({
