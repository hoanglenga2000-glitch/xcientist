...
async function localTrainingFallbackDisabled() {
  ...
  if (compute.execution_mode === "local" && compute.local_training_enabled === true) return false;
  if (process.env.WORKSTATION_DISABLE_LOCAL_TRAINING === "0") return false;
  if (process.env.WORKSTATION_DISABLE_LOCAL_TRAINING === "1") return true;
