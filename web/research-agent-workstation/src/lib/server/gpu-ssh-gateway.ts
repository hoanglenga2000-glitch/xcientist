...
function workspacePolicyViolation(remoteWorkspace: string) {
  ...
  if (normalized === "/hpc2hdd/home/aimslab") {
    return "GPU remote workspace points at the shared home root. Use /hpc2hdd/home/aimslab/jinghw/scripts/gpu_tra for workstation jobs.";
  }
  const allowedRemoteWorkspaces = new Set([
    "/hpc2hdd/home/aimslab/jinghw/scripts/gpu_tra",
    "/hpc2hdd/home/aimslab/research_agent_workstation"
  ]);
  if (!allowedRemoteWorkspaces.has(normalized)) {
    return "GPU remote workspace must be the dedicated Jing workspace /hpc2hdd/home/aimslab/jinghw/scripts/gpu_tra, not a shared home or task root.";
  }
  return "";
}
...
async function rejectWorkspacePolicyViolation(action: string): Promise<GpuGatewayResult | null> {
  ...
  const artifact = await writeJsonArtifact(`workspace/gpu/workspace_policy_blocked_${stamp()}.json`, {
    ...
    required_remote_workspace: "/hpc2hdd/home/aimslab/jinghw/scripts/gpu_tra",
    training_started: false,
    policy: "Do not write workstation scripts, logs, datasets, or results directly under /hpc2hdd/home/aimslab."
  });
  await logAction({
    action: "gpu_workspace_policy_blocked",
    message: violation,
    artifactPath: artifact,
    metadata: { remote_workspace: config.remoteWorkspace, required_remote_workspace: "/hpc2hdd/home/aimslab/jinghw/scripts/gpu_tra" }
  });
  return {
    ok: true,
...
function sshBaseArgs() {
  const config = gpuSshConfig();
  const hostKeyArgs = config.knownHostsPath
...
function proxyCommand(host: string, port: string, username: string) {
  const scriptPath = proxyCommandScriptPath();
  const userArg = username ? ` ${username}` : "";
  const python = pythonCommand();
  if (process.platform === "win32") {
    return `cmd /c cd /d "${path.dirname(scriptPath)}" && "${python}" ${path.basename(scriptPath)} ${host} ${port} %h %p${userArg}`;
  }
  return `${python} "${scriptPath}" ${host} ${port} %h %p${userArg}`;
}

function hostKeyPolicy() {
  return gpuSshConfig().knownHostsPath ? "known_hosts_verified" : "trust_on_first_use";
}
