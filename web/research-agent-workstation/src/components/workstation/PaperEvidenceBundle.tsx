    ...
    <Card className="border-accent-light">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="overflow-hidden rounded-md border border-edge">
          <div className="grid grid-cols-[1.1fr_.9fr_.9fr_.9fr_.9fr_1.2fr] bg-surface-sunken px-3 py-2 text-[11px] font-bold uppercase text-ink-muted">
            <span>Task</span>
            <span>Round1</span>
            <span>Round2</span>
          ...
          {trajectory.length ? trajectory.map((row) => (
            <div key={row.task_id} className="grid grid-cols-[1.1fr_.9fr_.9fr_.9fr_.9fr_1.2fr] items-center border-t border-edge-light px-3 py-2 text-xs">
              <span className="font-bold text-ink">{row.task_id}</span>
            ...
            <div className="border-t border-edge-light px-3 py-6 text-sm text-ink-muted">No trajectory loaded.</div>
          )}
        </div>
        ...
        <div className="rounded-md border border-warning/25 bg-warning-light p-3 text-xs leading-5 text-warning-text">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-md border border-edge bg-surface-sunken px-3 py-2 text-xs font-semibold text-ink-secondary">
...
function EvidenceMetric({
  ...
  return (
    <div className="rounded-md border border-edge bg-surface-raised p-3">
      ...
      <div className="mt-3 text-[11px] font-bold uppercase text-ink-muted">{label}</div>
      <div className="mt-1 text-xl font-bold text-ink">{String(value)}</div>
    </div>
  );
}
...
function LayerCard({ title, subtitle, body }: { title: string; subtitle: string; body: string }) {
  return (
    <div className="rounded-md border border-edge bg-surface-raised p-4">
      <div className="text-xs font-bold uppercase text-accent-dark">{title}</div>
      <div className="mt-1 text-sm font-bold text-ink">{subtitle}</div>
      <p className="mt-2 text-xs leading-5 text-ink-secondary">{body}</p>
    </div>
  );
}
...
function SteadyImprovementPanel({ bundle }: { bundle: PaperEvidenceBundleType | null }) {
  ...
  return (
    <div className="rounded-md border border-success/25 bg-success-light/60 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-xs leading-5 text-success-text">
      ...
      {branches.length ? (
        <div className="mt-3 grid gap-2 lg:grid-cols-3">
          {branches.map((branch) => (
            <div key={branch.task_id} className="rounded-md border border-success/25 bg-surface-raised p-3 text-xs">
              <div className="flex items-center justify-between gap-2">
                <div className="font-bold text-ink">{branch.task_id}</div>
                <StatusBadge tone="blue">{branch.code_generation_mode ?? "mode"}</StatusBadge>
              </div>
              <div className="mt-2 text-[11px] font-bold uppercase text-ink-muted">{branch.search_stage ?? "search"}</div>
              <div className="mt-1 font-semibold text-ink">{branch.branch_type ?? "branch"}</div>
              <p className="mt-2 leading-5 text-ink-secondary">{branch.hypothesis ?? "Hypothesis pending."}</p>
            </div>
          ))}
        </div>
      ) : null}
  ...
  return (
    <div className="rounded-md border border-edge bg-surface-raised p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          <div className="text-sm font-bold text-ink">Paper Figures</div>
          <div className="mt-1 text-xs text-ink-muted">Figure manifest is attached to the same evidence bundle.</div>
        </div>
        <StatusBadge tone="green">figures ready</StatusBadge>
      </div>
      <div className="grid gap-3 lg:grid-cols-3">
        {figures.map((figure) => (
          <div key={figure.figure_id} className="rounded-md border border-edge bg-surface-sunken p-3 text-xs">
            <div className="font-bold text-ink">{figure.title}</div>
            <p className="mt-2 min-h-14 leading-5 text-ink-secondary">{figure.caption}</p>
            <div className="mt-2 truncate rounded border border-edge bg-surface-raised px-2 py-1 font-mono text-[11px] text-ink-muted">
...
function VerificationGatePanel({ bundle }: { bundle: PaperEvidenceBundleType | null }) {
  ...
  return (
    <div className="mt-3 rounded-md border border-accent-light bg-surface-raised p-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <div className="text-xs font-bold uppercase text-accent-dark">Machine Verification Gate</div>
          <p className="mt-1 text-xs leading-5 text-ink-secondary">{verification.claim_verified ?? "Verification artifact loaded."}</p>
        </div>
        <StatusBadge tone={passed ? "green" : "red"}>{verification.status ?? "pending"}</StatusBadge>
      </div>
      <div className="mt-3 grid gap-2 md:grid-cols-3">
        {checks.slice(0, 9).map((check) => (
          <div key={check.id} className="rounded border border-edge bg-surface-sunken px-2 py-2 text-xs">
            <div className="flex items-center justify-between gap-2">
              <span className="truncate font-bold text-ink">{check.id}</span>
              <StatusBadge tone={check.passed ? "green" : "red"}>{check.passed ? "pass" : "fail"}</StatusBadge>
            </div>
            <p className="mt-1 leading-4 text-ink-muted">{check.description}</p>
          </div>
        ))}
      </div>
      <div className="mt-2 truncate rounded border border-edge bg-surface-sunken px-2 py-1 font-mono text-[11px] text-ink-muted">
