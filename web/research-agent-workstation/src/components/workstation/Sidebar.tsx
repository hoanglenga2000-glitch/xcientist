...
import { deriveConnectorDisplays, type ConnectorTone } from "@/lib/connector-status";
...
const connectorToneClass: Record<ConnectorTone, string> = {
  green: "text-emerald-300",
  amber: "text-amber-300",
  red: "text-rose-300",
  slate: "text-blue-100/70"
};
...
export function Sidebar({
  ...
  const connectorRows = [
    ...
    connectorDisplays.humanGate
  ];
  const operationalRows = connectorRows.slice(0, 3);
  const connectorHeaderTone: ConnectorTone = operationalRows.every((row) => row.tone === "green")
    ? "green"
    : operationalRows.some((row) => row.tone === "red")
      ? "red"
      : "amber";
      ...
      <div className="flex h-full flex-col bg-[linear-gradient(180deg,#061326_0%,#071a32_58%,#061326_100%)] p-3">
        ...
        <div className={cn("mt-3 shrink-0 space-y-2 lg:block", !mobileOpen && "hidden lg:block")}>
          <div className="rounded-md border border-white/12 bg-white/[0.07] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className={cn(
                ...
                connectorHeaderTone === "green" && "bg-emerald-400",
                connectorHeaderTone === "amber" && "bg-amber-300",
                connectorHeaderTone === "red" && "bg-rose-400"
              )} />
            </div>
            <div className="mt-2 space-y-1">
              {connectorRows.map((row) => (
                <div key={row.name} className="flex items-center justify-between gap-2 text-xs" title={row.detail}>
                  <span className="truncate text-blue-50/86">{row.name}</span>
                  <span className={cn("shrink-0 whitespace-nowrap font-black", connectorToneClass[row.tone])}>{row.state}</span>
                </div>
              ))}
            </div>
            <button
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2 text-[11px] font-semibold text-blue-100/70">
            <span>{text(locale, "Version not reported", "版本未报告")}</span>
